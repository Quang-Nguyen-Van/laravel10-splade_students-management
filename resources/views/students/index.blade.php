    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <x-splade-table :for="$students">
                @cell('avatar', $student)
                    @if ($student->avatar)
                        <img src="{{ asset($student->avatar) }}" alt="{{ $student->name }}" class="w-10 h-10 rounded-full" />
                    @endif
                @endcell
                @cell('actions', $student)
                    @can('student_edit')
                        <Link slideover href="{{ route('students.edit', $student) }}" class="px-3 py-2 bg-green-400 hover:bg-green-600 rounded m-2 text-white">
                    ...
                    @can('student_delete')
                        <Link href="{{ route('students.destroy', $student) }}" method="DELETE"
                            confirm="Delete the student"
